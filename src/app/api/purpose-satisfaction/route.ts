...
export async function GET(request: NextRequest) {
  ...
  if (rangeResult) {
    const data = await getPurposeSatisfaction(clinicId, rangeResult.days, rangeResult.range, attrFilters, cutoff ?? undefined)
  ...
  const data = await getPurposeSatisfaction(clinicId, days, undefined, attrFilters, cutoff ?? undefined)
  return successResponse(data)
}
